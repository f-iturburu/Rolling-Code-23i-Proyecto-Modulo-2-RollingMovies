...
let addMovieModal = new bootstrap.Modal(
...
function validateTitleAdd() {
  ...
  if (arrayMovies != []) {
    let movieFound = arrayMovies.find(
      (movie) => movie.name.toLowerCase() == tituloInput.value.toLowerCase()
    );
...
function validateDescriptionAdd() {
  ...
  if (descInput.value.length <= 20) {
    ...
    characterCount.className =
      "form-text mb-2 ms-auto col-4 text-end text-danger";
  } else {
...
export function validateGeneralAdd(e) {
  ...
  if (
    validateTitleAdd() &
    validateDescriptionAdd() &
    validateUrlImageAdd() &
    validateUrlFeaturedAdd() &
    validateUrlVideoAdd()
  ) {
...
function validateTitleEdit() {
  ...
  if (arrayMovies != []) {
    ...
    let movieFound = arrayMovies.find(
      (movie) => movie.name.toLowerCase() == tituloInputEdit.value.toLowerCase()
    );

    let currentMovie = arrayMovies.find(
      (movie) => movie.code == editModal.title
    );

    if (
      movieFound !== undefined &&
      movieFound.name.toLowerCase() == currentMovie.name.toLowerCase()
    ) {
...
export function validateGeneralEdit(e) {
  ...
  if (
    ...
    validateUrlImageEdit() &
    validateUrlFeaturedEdit() &
    validateUrlVideoEdit()
  ) {
